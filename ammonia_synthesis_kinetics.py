...
class AmmoniaSynthesisKinetics:
    ...
    def compute_adsorption_constants(self, T):
        ...
        """
        self.K_H2 = np.exp(-56.9024 / self.Ra + 37656 / (self.Ra * T))
        self.K_NH3 = np.exp(-34.7272 / self.Ra + 29228 / (self.Ra * T))
    ...
    def compute_fugacity_coeffs(self, T, p):
        ...
        p_loc = p.reshape(p.shape + (1,) * (ndim -1 - p.ndim))
    ...
    def __call__(self, c, T=None, p=None):
        """
        Compute the reaction rates for ammonia synthesis.
        ...
        --------
        numpy.ndarray
            Reaction rates for ammonia synthesis.
        """
        self.set_T_and_p(T, p)
        slices = [slice(None)] * c.ndim
        slices[self.axis] = [self.index_H2, self.index_N2, self.index_NH3]
        c_species = np.take(c, (self.index_H2, self.index_N2, self.index_NH3), axis=self.axis)
        p_partial = np.expand_dims(self.p, self.axis) * c_species / np.sum(c, axis=self.axis, keepdims=True)

        activities = p_partial * self.fugacity_coeffs
