"""Reaction-rate kinetics for ammonia synthesis.

Provides :class:`AmmoniaSynthesisKinetics`, which evaluates equilibrium,
rate, and adsorption constants plus fugacity coefficients as functions of
temperature and pressure, and returns volumetric reaction rates for the
H2/N2/NH3 system.
"""
...
class AmmoniaSynthesisKinetics:
    ...
    abs(self, c, c_small=C_SMALL):
        Compute the absolute value of concentrations with a small constant to avoid issues with zero.

    pow(self, c, n, c_small=C_SMALL):
    ...
    def abs(self, c, c_small=C_SMALL):
        """
        Compute the absolute value of concentrations with a small constant to avoid issues with zero.
        ...
        c_small : float, optional
            Small concentration to avoid issues with zero. Defaults to C_SMALL.

        Returns:
        --------
        numpy.ndarray
            Absolute value of concentrations with a small constant added.
        """
        return np.abs(c + c_small) + c_small

    def pow(self, c, n, c_small=C_SMALL):
        """
        Compute the power of concentrations with a small constant to avoid division by zero.
        ...
        """
        return c * (self.abs(c, c_small=c_small)) ** (n - 1)
        ...
        rate = guarded_compute(
            ...
            lambda: (
                ...
                * (
                ...
                / (
                    1.0
                    + self.K_H2 * self.abs(a_H2) ** 0.3
                    + self.K_NH3 * self.abs(a_NH3) ** 0.2
                )
            ),
