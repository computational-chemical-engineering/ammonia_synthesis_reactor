"""
1D axisymmetric membrane reactor model for ammonia synthesis.

This module implements a cross-sectionally averaged (1D) membrane reactor,
mirroring the architecture of the 2D model in membrane_reactor.py.  The key
...
class MembraneReactor1D:
    ...
    def _init_derived(self):
        ...
        # Species transfer uses one common membrane area only
        self.factor_geom_mem = 1.0

        logger.debug(
            "1D geometry: r_min=%g, r_max_perm=%g, A_ret=%g, A_perm=%g, "
            "S_mem_ret=%g, S_mem_perm=%g, factor_geom_mem=%g, S_mem_perm_eff=%g",
            self.r_min, self.r_max_perm, self.A_ret, self.A_perm,
            self.S_mem_ret, self.S_mem_perm, self.factor_geom_mem,
            self.S_mem_perm * self.factor_geom_mem,
        )
    ...
    def _membrane_flux(self, c, T):
        """Compute membrane species flux J_i and its Jacobian contributions.

        J_i = Perm_i * Rg * (T_ret * c_i_ret - T_perm * c_i_perm)

        Returns (g_mem, jac_mem) where:
          g_mem: (num_z, 2, num_c)  contribution to species residual
          jac_mem: csc_array in monolithic cpT shape
        """
        nz, nc = self.num_z, self.num_c
        c_ret = c[:, 1, :]     # (nz, nc)
        c_perm = c[:, 0, :]    # (nz, nc)
        ...
        # J_i = Perm_i * Rg * (T_ret * c_ret_i - T_perm * c_perm_i)
        # Positive J means flow from retentate to permeate (species leaves ret)
        Rg = self.Rg
        perm = self.P0 * np.exp(-self.EA / (Rg * T_ret[:, None]))  # (nz,nc)
        J = perm * Rg * (T_ret[:, None] * c_ret - T_perm[:, None] * c_perm)  # (nz, nc)

        g_mem = np.zeros((nz, 2, nc))
        # Retentate loses species: +S_mem_ret * J  (positive = loss, added to residual)
        g_mem[:, 1, :] = self.S_mem_ret * J
        # Permeate gains species: -S_mem_perm * factor_geom * J
        g_mem[:, 0, :] = -self.S_mem_perm  * J

        # ---- Analytical Jacobian ----
        # Coefficient arrays (nz, nc)
        coeff_ret_ret = self.S_mem_ret * perm * Rg * T_ret[:, None]      # dg_ret / dc_ret
        coeff_ret_perm = -self.S_mem_ret * perm * Rg * T_perm[:, None]   # dg_ret / dc_perm
        coeff_perm_ret = -self.S_mem_perm * perm * Rg * T_ret[:, None]
        coeff_perm_perm = self.S_mem_perm  * perm * Rg * T_perm[:, None]
        ...
        # Build 4 diagonal blocks, each (nz*1*nc)×(nz*1*nc), then embed
        # 1) ret←ret
    ...
    def _construct_g_cpT(self, c_old, T_old, dt, compute_jac=False):
        ...
        if compute_jac:
            ...
            c_tot, dc_tot_dp = self.numjac_p(
                lambda pv: self.correlation.molar_density(y, T, pv), p
            )
            _, dc_tot_dT = self.numjac_p(
                lambda Tv: self.correlation.molar_density(y, Tv, p), T, f_value=c_tot
            )
            jac_pp = dc_tot_dp
            jac_pc = -self.sum_c
            jac_pT = dc_tot_dT
            ...
            if self.is_isothermal:
                ...
                jac_TP = jac_T_darcy
                # Membrane heat coupling
                if jac_Tmem is not None:
                    # jac_Tmem is in (nz,2) temperature space — needs to go into cpT
                    pass  # will add after offset mapping

                # Thermo-chemical cross-coupling
                shape_T_ret_loc = (nz, 1, 1)
                shape_c_ret_loc = (nz, 1, nc)

                _, jac_cT_react = self.numjac_cT(
                    lambda Tv: self.factor_react * self.kinetics(
                        self._reaction_partial_pressures(c_ret, Tv[..., 0], p_ret), Tv[..., 0],
                    ), T_ret[..., np.newaxis],
                )
            ...
            jac_cp = update_csc_array_indices(jac_cp, (shape_c_full, shape_p_1), shape_cpT, offset=(None, offset_p))
            jac_pc = update_csc_array_indices(jac_pc, (shape_p_1, shape_c_full), shape_cpT, offset=(offset_p, None))
            jac_pT = update_csc_array_indices(jac_pT, (shape_p_1, shape_p_1), shape_cpT, offset=(offset_p, offset_T))
            ...
            if not self.is_isothermal:
                if jac_TP is not None:
                    jac_TP = update_csc_array_indices(jac_TP, shape_p_1, shape_cpT, offset=(offset_T, offset_p))
                    base_jac = base_jac + factor_T * jac_TP

                # Membrane heat coupling → cpT
                if jac_Tmem is not None:
            ...
            if not self.is_isothermal:
                enthalpies = self.correlation.species_enthalpies(T_ret)
                _, _, _, cp_inv_perm_v, cp_inv_ret_v = self._construct_g_T_cond(T, c, jac_only=False)
                cp_inv_ret_flat = cp_inv_ret_v.ravel()
                g_T_react = (
        ...
        if self.is_isothermal:
            ...
            if T_old is not None:
                g_T += (self.jac_T_accum @ ((T - T_old).reshape((-1, 1)) / dt)).reshape(T.shape)
            g_T[:, 1:] += g_T_react
    ...
    def compute_flows(self):
        ...
        # Molar flux = c * u * A → flow = flux * A
        # For 1D: F_i(z_face) = c_i(z_face) * u(z_face) * A
        ...
        # Membrane transfer: integrate J_i over membrane area
        T_ret = T[:, 1]
        T_perm = T[:, 0]
        c_ret_cell = c[:, 1, :]   # (nz, nc)
        c_perm_cell = c[:, 0, :]  # (nz, nc)
        Rg = self.Rg
        perm = self.P0 * np.exp(-self.EA / (Rg * T_ret[:, None]))  # (nz,nc)
        J = perm * Rg * (T_ret[:, None] * c_ret_cell - T_perm[:, None] * c_perm_cell)
        # J has shape (nz, nc), membrane area per cell = 2 pi r_min * dz
