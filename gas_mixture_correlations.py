...
class GasMixtureCorrelations:
    ...
    def __init__(self, species, filename):
        ...
        self.mi = np.where(self.omega < 0.491, 0.37464+1.54226*self.omega-0.26992*self.omega**2, 0.379642+1.48503*self.omega-0.164423*self.omega**2+0.016666*self.omega**3)
        R = constants.R
    ...
    def molar_density(self, y, T, p, axis=-1):
        ...
        T = np.asarray(T)
        p = np.asarray(p)
        shape_t_y, shapes_t, _, shape_out = get_tri_shapes(shapes_species = y.shape, shapes_nonspecies = [T.shape, p.shape], axis=axis)
    ...
    def viscosity(self, y, T, axis=-1):
        ...
        mu_i = self.wilke_C1 * T_t**self.wilke_C2 * (1 + self.wilke_C3 / T_t + self.wilke_C4 /T_t**2)  # Shape: same as y
        ...
        phi_ij = ((1 + (mu_i_exp / mu_j_exp)**0.5 * (Mw_i_exp / Mw_j_exp)**0.25)**2) / \
    ...
    def thermal_conductivity(self, y, T, axis=-1):
        ...
        kg_i = self.therm_cond_C1 * T_t**self.therm_cond_C2 * (1 + self.therm_cond_C3 / T_t + self.therm_cond_C4 /T_t**2)          
    ...
    def get_species_specific_heat_spline(self, T, dT=5.0, T_ref = None):
        ...
        if hasattr(self, "c_p_spline") and T_min >= self.c_p_spline.x[0] and T_max <= self.c_p_spline.x[-1]:
            return self.c_p_spline
                
        def c_p_func(T):
            T_lin = T.reshape((-1,1))
            return 1e-3*(self.c_p_C1 + self.c_p_C2 * ((self.c_p_C3 / T_lin) / np.sinh(self.c_p_C3 / T_lin))**2 
                         + self.c_p_C4 * ((self.c_p_C5 / T_lin) / np.cosh(self.c_p_C5 / T_lin))**2)
        ...
        self.c_p_spline = CubicSpline(range_T, c_p_func(range_T), axis=0, bc_type='natural', extrapolate=True)
        return self.c_p_spline
    ...
    def species_specific_heat(self, T, axis=-1):
        ...
        shape_species = (1, self.num_species, 1)
    ...
    def species_enthalpies(self, T, T_ref = 298.15, dT= 5.0, axis=-1):
        ...
        H = np.permute_dims(H, (0,2,1)) - H_spline(T_ref).reshape((1,-1,1)) + self.dH_f
    ...
    def enthalpy(self, y, T, axis=-1):
        ...
        y_t = y.reshape(shape_t_y)
        T_t = T.reshape(shape_t_T)
        H_s = self.species_enthalpies(T, axis=axis).reshape((shape_t_T[0], -1, shape_t_T[2]))
    ...
    def diffusion(self, y, T, p, axis=-1):
        ...
        T = np.asarray(T)
        p = np.asarray(p)
        shape_t_y, shapes_t, shape_out,_ = get_tri_shapes(shapes_species = y.shape, shapes_nonspecies = [T.shape, p.shape], axis=axis)
        y_t = y.reshape(shape_t_y)
        T_t = T.reshape(shapes_t[0])
        p_t = p.reshape(shapes_t[1])
...
def get_tri_shapes(shapes_species=None, shapes_nonspecies=None, axis=-1):
    ...
    ndim = max(
        ...
        max((len(shape) + 1 for shape in shapes_nonspecies), default=0) if has_nonspecies else 0
    )
    ...
    if has_species:
        for shape in shapes_species:
            ...
            shape_t = (math.prod(shape), 1, 1) if axis >= len(shape) else (math.prod(shape[:axis]), shape[axis], math.prod(shape[axis+1:]))
    ...
    if has_species and has_nonspecies:
        return shapes_t_species, shapes_t_nonspecies,  tuple(shape_out), shape_out_nonspecies
    elif has_species:
